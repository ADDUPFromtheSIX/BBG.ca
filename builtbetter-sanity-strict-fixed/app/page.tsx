...
async function getHomepageData(): Promise<HomepageData> {
  try {
    const data = (await client.fetch(homepageQuery)) as HomepageData | null;
...
async function getTestimonials(): Promise<Testimonial[]> {
  try {
    const data = (await client.fetch(testimonialsQuery)) as Testimonial[] | null;
...
export default async function HomePage() {
  ...
  const bgUrl = homepage.heroBackgroundImage
    ? urlFor(homepage.heroBackgroundImage).width(1800).height(1000).url()
    : "https://images.unsplash.com/photo-1600585154340-be6161a56a0c";

  const specialties: Specialty[] = homepage.specialties || [];
  const processSteps: ProcessStep[] = homepage.processSteps || [];
    ...
    <div className="min-h-screen bg-white text-gray-900">
      <header className="flex flex-col gap-4 border-b border-gray-800 bg-black px-5 py-5 text-white sm:flex-row sm:items-center sm:justify-between sm:px-8 sm:py-6">
        ...
        <div className="text-left sm:text-right">
          ...
          <a href="tel:4377999675" className="text-lg font-semibold transition hover:text-yellow-400">
            437-799-9675
          </a>
        </div>
      </header>
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl">
          <h1 className="mb-6 whitespace-pre-line text-4xl font-bold leading-tight sm:text-5xl md:text-6xl">
            {homepage.heroTitle}
          </h1>
          ...
          <a
            ...
            className="inline-flex w-full items-center justify-center rounded-2xl bg-yellow-500 px-6 py-4 font-medium text-black transition hover:opacity-90 sm:w-auto sm:px-8"
          >
          ...
          {specialties.map((specialty: Specialty) => (
            <div key={specialty.title} className="rounded-2xl border border-gray-800 bg-black/40 p-6">
              ...
              <div className="space-y-3 text-sm text-gray-300">
                {specialty.items?.map((item: string) => (
      ...
      <section id="project-contact-form" className="mx-auto max-w-3xl px-5 py-16 sm:px-8 sm:py-20">
        ...
        <form action="https://formspree.io/f/maqapqqe" method="POST" className="grid gap-4">
          ...
          <input
            ...
            required
            className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-gray-900 outline-none transition focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20"
          />
          <input
            name="phone"
            placeholder="Phone"
          ...
          <textarea
            ...
            className="min-h-32 w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-gray-900 outline-none transition focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20"
          />
          <button
            type="submit"
            className="inline-flex w-full items-center justify-center rounded-2xl bg-yellow-500 px-6 py-4 font-medium text-black transition hover:opacity-90 sm:w-auto"
          >
            Submit
          </button>
        </form>
      ...
      <footer className="border-t border-gray-800 bg-black p-6 text-center text-white sm:p-8">
        ...
        <p className="text-sm text-gray-400">
          Condo Renovation, Garage Builds, Garden Suites & Outdoor Specialists - Toronto
        </p>
